...
std::queue<int> q1;
std::mutex q1Mutex;
std::condition_variable q1CondVar;
...
void provider (int val)
{
    ...
    {
        {
            std::lock_guard<std::mutex> lg(q1Mutex);
            q1.push(val + i);
        } // release lock
        q1CondVar.notify_one();
...
void consumer (int num)
{
    for (int cnt = 0; cnt < 9; ++cnt)
    {
        int val;
        {
            std::unique_lock<std::mutex> ul(q1Mutex);
            q1CondVar.wait(ul, [] { return !q1.empty(); });
            val = q1.front();
            q1.pop();
        } // release lock
